...
require('./createPackage.js')('mac', { arch: toArch(platform) }).then(function (packagePath) {
  if (platform === 'arm64') {
    execSync('codesign -s - -a arm64 -f --deep ' + packagePath + '/WaterWolf.app')
  }
  ...
  var output = fs.createWriteStream('dist/app/waterwolf-v' + version + '-mac-' + platform + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })

  archive.directory(path.resolve(packagePath, 'WaterWolf.app'), 'WaterWolf.app')
