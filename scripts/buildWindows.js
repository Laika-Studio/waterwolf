...
async function afterPackageBuilt (packagePath) {
  ...
  /* create zip files */
  var output = fs.createWriteStream('dist/app/' + 'WaterWolf-v' + version + '-windows' + archSuffix + '.zip')
  var archive = archiver('zip', {
    zlib: { level: 9 }
  })
  archive.directory(packagePath, 'WaterWolf-v' + version)
  archive.pipe(output)
  ...
  const options = {
    src: packagePath,
    dest: 'dist/app/waterwolf-installer' + archSuffix,
    icon: 'icons/icon256.ico',
    animation: 'icons/windows-installer.gif',
    licenseUrl: 'https://github.com/Laika-Studio/waterwolf/blob/master/LICENSE.txt',
    noMsi: true
  }
  ...
  await installer(options)
    .then(function () {
      fs.renameSync('./dist/app/waterwolf-installer' + archSuffix + '/waterwolf-' + version + '-setup.exe', './dist/app/waterwolf-' + version + archSuffix + '-setup.exe')
    })
    .catch(err => {
