...
see https://github.com/Laika-Studio/waterwolf/issues/657 for more information */
...
if (settings.get('customUserAgent')) {
  ...
  newUserAgent = defaultUserAgent.replace(/WaterWolf\/\S+\s/, '').replace(/Electron\/\S+\s/, '').replace(process.versions.chrome, process.versions.chrome.split('.').map((v, idx) => (idx === 0) ? v : '0').join('.'))
}
