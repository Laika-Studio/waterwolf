...
const webviewMenu = {
  ...
  showMenu: function (data, extraData) { // data comes from a context-menu event
    ...
    translateMenu.submenu.push({
      label: 'Send Feedback',
      click: function () {
        browserUI.addTab(tabs.add({ url: 'https://github.com/Laika-Studio/waterwolf/issues/new?title=Translation%20feedback%20for%20' + encodeURIComponent(tabs.get(tabs.getSelected()).url) }), { enterEditMode: false, openInBackground: false })
      }
    })
