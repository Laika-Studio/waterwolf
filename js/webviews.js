...
function onPageURLChange (tab, url) {
  if (url.indexOf('https://') === 0 || url.indexOf('about:') === 0 || url.indexOf('chrome:') === 0 || url.indexOf('file://') === 0 || url.indexOf('waterwolf://') === 0) {
    tabs.update(tab, {
      secure: true,
      url: url
    })
...
const webviews = {
  ...
  asyncCallbacks: {},
  internalPages: {
    error: 'waterwolf://app/pages/error/index.html'
  },
  events: [],
  IPCEvents: [],
  ...
  add: function (tabId, existingViewId) {
    ...
    if (!existingViewId) {
      if (tabData.url) {
        ...
        ipc.send('loadURLInView', { id: tabData.id, url: urlParser.parse('waterwolf://newtab') })
      }
    }
...
settings.listen(function () {
  tasks.forEach(function (task) {
    task.tabs.forEach(function (tab) {
      if (tab.url.startsWith('waterwolf://')) {
        try {
...
webviews.bindIPC('downloadFile', function (tabId, args) {
  if (tabs.get(tabId).url.startsWith('wolf://')) {
